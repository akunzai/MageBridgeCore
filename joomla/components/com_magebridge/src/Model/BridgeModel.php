...
class BridgeModel
{
    ...
    public function build()
    {
        ...
        if ($this->isOffline()) {
            ...
            error_log('MageBridge Debug: Bridge is offline');
        ...
        if (strlen(ConfigModel::load('api_user')) == 0 && strlen(ConfigModel::load('api_key')) == 0) {
            ...
            error_log('MageBridge Debug: No API user or key');
        ...
        if ($proxy->getCount() > 10) {
            ...
            error_log('MageBridge Debug: Too many requests');

            return $register->getRegister();
        }

        error_log('MageBridge Debug: Pending count = ' . count($register->getPendingRegister()));
        ...
        if (count($register->getPendingRegister()) > 0) {
            ...
            //DebugModel::getInstance()->trace( 'Bridge-data', $data );
    ...
    public function getMageSession(): ?string
    {
        /** @var CMSApplication */
        $app = Factory::getApplication();
        $session_value = $app->getInput()->cookie->getCmd('frontend');

        if (empty($session_value)) {
            $session_value = $app->getSession()
    ...
    public function setMageSession(?string $mage_session = null): ?string
    {
        ...
        return $app->getInput()->cookie->getCmd('frontend');
    }

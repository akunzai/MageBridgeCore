    ...
    public function runOnPurchase($sku = null, $qty = 1, $user = null, $status = null, $arguments = null)
    {
        ...
        foreach ($conditions as $condition) {
            ...
            if (!empty($condition->connector) && !empty($condition->connector_value)) {
                $event = AbstractEvent::create('onMageBridgeProductConvertField', ['condition' => $condition, 'actions' => &$actions]);
            ...
            $event = AbstractEvent::create('onMageBridgeProductPurchase', ['actions' => $actions, 'user' => $user, 'status' => $status, 'sku' => $sku]);
            $dispatcher = Factory::getContainer()->get(DispatcherInterface::class);

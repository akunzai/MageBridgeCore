...
namespace MageBridge\Plugin\Magento\MageBridge;
...
use MageBridgeConnectorProduct;
use MageBridgeConnectorProfile;
 ...
 * @since  3.0.0
 */
class MagentoPlugin extends CMSPlugin
{
    /**
     * @var CMSApplication
     */
    ...
    public function mageCustomerLogoutAfter($arguments = [])
    {
        ...
        $event = AbstractEvent::create('onUserLogout', ['customer' => $customer, 'options' => $options]);
        $dispatcher = Factory::getContainer()->get(DispatcherInterface::class);
        $dispatcher->dispatch('onUserLogout', $event);
    ...
    public function mageCustomerLoginAfter($arguments = [])
    {
        ...
        // Do a post-login
        $this->getUser()->postlogin(null, $user->id, false);
    ...
    public function mageCustomerSaveAfter($arguments = [])
    {
        ...
        if (isset($customer['joomla_id'])) {
            $user = Factory::getContainer()->get(UserFactoryInterface::class)->loadUserById($customer['joomla_id']);
        }
        ...
        // Decrypt the user-password before continuing
        ...
        if (!empty($customer['name']) && !empty($customer['email'])) {
            ...
            $customer['usergroup'] = UserHelper::getJoomlaGroupIds($customer, $usergroups);
            ...
            if (isset($customer['is_subscribed'])) {
                ...
                $event = AbstractEvent::create('onNewsletterSubscribe', ['user' => $user, 'is_subscribed' => (bool)$customer['is_subscribed']]);
                $dispatcher = Factory::getContainer()->get(DispatcherInterface::class);
                $dispatcher->dispatch('onNewsletterSubscribe', $event);
            }
    ...
    public function mageNewsletterSubscriberChangeAfter($arguments = [])
    {
        ...
        // Run the newsletter plugins
        PluginHelper::importPlugin('magebridgenewsletter');
        $event = AbstractEvent::create('onNewsletterSubscribe', ['user' => $user, 'state' => $state]);
        $dispatcher = Factory::getContainer()->get(DispatcherInterface::class);
        $dispatcher->dispatch('onNewsletterSubscribe', $event);

...
namespace MageBridge\Plugin\MageBridgeStore\Joomla\Extension;
 ...
 *
 * Dynamically loads a Magento store-scope based on the Joomla language.
 *
 * @since 4.0.0
 */
class JoomlaStorePlugin extends CMSPlugin implements SubscriberInterface
{
    /**
     * Database object.
     */
     ...
     */
    protected string $connector_field = 'joomla_language';
     ...
     *
     * Validates if the current Joomla language matches the configured store mapping.
     *
    ...
    public function onMageBridgeValidate(?array $actions = null, ?object $condition = null): bool
    {
        ...
        $languageCode = $this->app->getInput()->getCmd('language', '');
        if ($actions['joomla_language'] === $languageCode) {
            return true;
        }

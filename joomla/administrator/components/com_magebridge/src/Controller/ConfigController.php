...
class ConfigController extends BaseController
{
    ...
    private function processStore(array $data = []): array
    {
        ...
        try {
            $model->store($post);
            $message = Text::_('LIB_YIREO_CONTROLLER_ITEM_SAVED');

            return ['success' => true, 'message' => $message, 'type' => 'message'];
        } catch (Exception $e) {
            $message = Text::_('LIB_YIREO_CONTROLLER_ITEM_NOT_SAVED');
            $error   = $e->getMessage();
    ...
    {
        $post['api_key']  = $this->input->post->get('api_key', '', 'raw');
        $post['api_user'] = $this->input->post->get('api_user', '', 'raw');

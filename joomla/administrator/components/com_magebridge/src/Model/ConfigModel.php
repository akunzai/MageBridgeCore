 ...
 */
class ConfigModel extends \Yireo\Model\AbstractModel
{
    /**
    ...
    private function loadDefaultConfig()
    {
        foreach ($this->getDefaults() as $name => $value) {
            $this->config[$name] = (new SiteConfigValue(['name' => $name, 'value' => $value]))->toArray();
        }
            ...
            if (!empty($data)) {
                foreach ($data as $d) {
                    if ($d->name == $c['name']) {
                        ...
                        $this->config[$name] = (new SiteConfigValue((array) $d))->toArray();
                        break;
                    }
                }
            }
    ...
    {
        $this->config['method'] = (new SiteConfigValue(['value' => 'post']))->toArray();
        ...
        if ($this->config['update_format']['value'] == '') {
            ...
            $this->config['update_format'] = (new SiteConfigValue(['value' => $value]))->toArray();
        }

        // Disable widgets if needed
        if ($this->input->getInt('widgets', 1) == 0) {
            $this->config['api_widgets'] = (new SiteConfigValue(['value' => 0]))->toArray();
        }
        ...
        if ($this->app->isClient('administrator') && $this->input->getCmd('option') == 'com_magebridge' && $this->input->getCmd('view') == 'root') {
            //$this->config['debug'] = (new SiteConfigValue(['value' => 0]))->toArray();
            $this->config['disable_js_all'] = (new SiteConfigValue(['value' => 1]))->toArray();
            $this->config['disable_js_mootools'] = (new SiteConfigValue(['value' => 1]))->toArray();
        }
        ...
        if (!isset($this->config['port'])) {
            $this->config['port'] = (new SiteConfigValue(['value' => $port]))->toArray();
        }
        ...
        if (!isset($this->config['url'])) {
            ...
            $this->config['url'] = (new SiteConfigValue(['value' => $url]))->toArray();
        }

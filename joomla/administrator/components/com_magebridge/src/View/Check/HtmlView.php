    ...
    public function displayResult($tpl)
    {
        ...
        if (!preg_match('/^([0-9\.]+)$/', $host)) {
            ...
            if (gethostbyname($host) === $host) {
                throw new RuntimeException('ERROR: Failed to resolve hostname "' . $host . '" in DNS');
            }
        }

        if (@fsockopen($host, $port, $errno, $errmsg, 5) === false) {
            throw new RuntimeException('ERROR: Failed to open a connection to host "' . $host . '" on port "' . $port . '". Perhaps a firewall is in the way?');
        }

...
class Query
{
    ...
    {
        // Reset the query and where/orderby clauses to avoid duplicates when build() is called multiple times
        $this->query = $this->db->getQuery(true);
        $this->where = [];
        $this->orderby = [];
    ...
    protected function buildWhere()
    {
        ...
        if ($state == 'U' || $state == 'P') {
            ...
            if (!empty($stateField)) {
                $this->addWhere($this->db->quoteName($this->tableAlias) . '.`' . $stateField . '` = ' . $this->db->quote((string)$state));
            }
        }
    ...
    protected function buildQueryOrderBy()
    {
        ...
        if (!empty($cleanedOrderBy)) {
            $this->query->order(implode(', ', $cleanedOrderBy));
        }
    ...
    {
        // Skip limit if no_limit is set (used for COUNT queries)
        if ($this->getConfig('no_limit')) {
            return $this;
        }
     ...
     * @param string $type Type of WHERE statement. Either "is", "like", or "raw".
     *
     * @return $this
     */
    public function addWhere($where, $type = 'is')
    {
        ...
        if (is_array($where) && count($where) == 2 && $type !== 'raw') {
            if ($type == 'like') {
    ...
    public function addOrderby($orderby = null)
    {
        ...
        // Store in config so it persists through build() calls
        $currentOrderby = $this->getConfig('orderby') ?: [];
        if (!in_array($orderby, $currentOrderby)) {
            $currentOrderby[] = $orderby;
            $this->setConfig('orderby', $currentOrderby);
        }

...
class ModelItem extends DataModel
{
    ...
    protected function initParams()
    {
        ...
        // Only SiteApplication has getParams() method
    ...
    public function getData($forceNew = false)
    {
        ...
        if ($this->getId() > 0) {
            ...
            if ($this->app->isClient('site') && isset($data->$stateField) && $data->$stateField == 0) {
                throw new NotFound(Text::_('LIB_YIREO_MODEL_NOT_FOUND'));
            }
    ...
    public function delete($cid = [])
    {
        ...
        $query->where($db->quoteName($primaryKey) . ' IN (' . $cids . ')');
    ...
    protected function getEmpty()
    {
        ...
        if (empty($this->data)) {
            // Get table properties using reflection instead of deprecated getProperties()
            $tableProperties = [];
            $reflection = new ReflectionObject($this->table);
            foreach ($reflection->getProperties(ReflectionProperty::IS_PUBLIC) as $property) {
                if (!$property->isStatic()) {
                    $propertyName = $property->getName();
                    $tableProperties[$propertyName] = $this->table->$propertyName;
                }
            }
            $this->data = (object) $tableProperties;
